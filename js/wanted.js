...
function wantedLoadTierImages(rows) {
  ...
  const loadPortrait = async (name) => {
    const portrait = await loadImg(wantedGetPortraitPath(name));
    if (portrait) return portrait;
    return await loadImg("sprites/branding/logo_0.png");
  };

  const promises = [...names].map(async (name) => {
    const { weaponIcon } = wantedParseCounterName(name);
    const [portrait, weapon] = await Promise.all([
      loadPortrait(name),
      weaponIcon ? loadImg(weaponIcon) : Promise.resolve(null),
    ]);
    return [name, { portrait, weapon }];
  });

  return Promise.all(promises).then((entries) => new Map(entries.filter((e) => e[1].portrait)));
}
